...
class QANN(AbstractProcess):
    def __init__(
        ...
        *,
        ...
        self.activation = Var(shape=(1,), init=activation)
...
if loihi_available:
    ...
    class NcModelQANN(AbstractNcProcessModel):
        ...
        activation: NcVar = LavaNcType(NcVar, bool, precision=1)
        ...
        def allocate(self, net: NetL2):
            ...
            activation = self.activation.var.get()
            ...
            if np.isscalar(
                ...
                if interval_sub_1 == 0:  # If No reset
                    if vth > 0:
                        ...
                        neuron_cfg_params["ucode"] = ucode_file
                        neuron_cfg_params["vth"] = vth
                        neuron_cfg_params["scale"] = scale
                    else:
                    ...
                    if vth > 0:
                        ...
                        neuron_cfg_params["ucode"] = ucode_file
                        neuron_cfg_params["vth"] = vth
                        neuron_cfg_params["interval_sub_1"] = interval_sub_1
                        neuron_cfg_params["scale"] = scale
                    else:
                        ucode_file = os.path.join(curr_dir, "qann_reset.dasm")
                        neuron_cfg_params["ucode"] = ucode_file
                        neuron_cfg_params["interval_sub_1"] = interval_sub_1
                        neuron_cfg_params["scale"] = scale
            else:  # If we are using channel wise scaling
                ucode_file = os.path.join(curr_dir, "qann_thresh_chan.dasm")
                neuron_cfg_params["ucode"] = ucode_file
                neuron_cfg_params["vth"] = vth
                neuron_params["scale"] = scale
                neuron_params["nothing1"] = 0
                neuron_params["nothing2"] = 0

            if not activation:
                # Replace _reset.dasm with _reset_norelu.dasm
                neuron_cfg_params["ucode"] = neuron_cfg_params["ucode"].replace(
                    ".dasm", "_norelu.dasm"
                )

            print(f"Using {neuron_cfg_params['ucode']}")
            neurons_cfg: Nodes = net.neurons_cfg.allocate_ucode(
                **neuron_cfg_params
            )
            neurons: Nodes = net.neurons.allocate_ucode(**neuron_params)
